(function($) {
    ...
    var defaults = {
        horizontal: false,
        // Optional UI effect. If set, we will try to use the jquery UI effects
        // to transition.
        // Expected format:
        // effect: {
        //     name: 'blind',
        ...
        // Core jQuery animation options.
        sAnimation: null,
        sAnimationDuration: 'normal',
        sAnimationShow: {},
        sAnimationHide: {},
    ...
    $js.fn.extend({
        ...
        select: function(idx) {
            if (e = this.items.get(idx)) {
                ...
                if (this.options.spotliteEffect) {
                    var effect = this.options.spotliteEffect;

                    // Do this the UI way.
                    $('div', this.spotlite)
                    .hide(effect.name, effect.options, effect.speed, function() {
                        $(this).remove();
                    });
                    $('<div>')
                    .html($(".feature", e).html())
                    .show(effect.name, effect.options, effect.speed, function() {
                        //
                    });
                }
                /* Completly custom animation */
                else if (this.options.sAnimation) {
                    $('.jspotlite-spotlite-content', this.spotlite)
                    .animate(self.options.sAnimationHide, self.options.sAnimationDuration, self.options.sAnimation, function() {
                        $(this).remove();
                    });
                    ...
                    .animate(self.options.sAnimationShow, self.options.sAnimationDuration, self.options.sAnimation, function() {
                        //
                    });
                }
                else if (typeof this.options.animate == 'function') {
                    this.options.animate(this, idx);
                }
                else {
